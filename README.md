...
A token is a credential issued upon successful authentication. <code style="color : Aqua">Every operation in Vault requires a token.</code> The `token_accessor` is a reference that can be used to manage the token (renew, revoke, lookup) without knowing the token itself.
